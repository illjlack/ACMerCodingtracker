...
public class GlobalExceptionHandler {
        ...
        /**
         * 处理Token失效异常 - 返回401，提示前端重新登录
         */
        @ExceptionHandler(TokenExpiredException.class)
        public ResponseEntity<ApiResponse<Map<String, Object>>> handleTokenExpiredException(TokenExpiredException e) {
                logger.warn("Token expired for platform {}: {}", e.getPlatform(), e.getMessage());
                Map<String, Object> data = new HashMap<>();
                data.put("platform", e.getPlatform().name());
                data.put("errorCode", e.getErrorCode());
                data.put("needReauth", true);
                return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                                .body(new ApiResponse<>(false, "认证失效，请重新登录", data));
        }
        ...
        public ResponseEntity<ApiResponse<Map<String, Object>>> handleNetworkException(NetworkException e) {
                logger.error("Network error for platform {}: {}", e.getPlatform(), e.getMessage(), e);
                Map<String, Object> data = new HashMap<>();
                data.put("platform", e.getPlatform().name());
                data.put("errorCode", e.getErrorCode());
                return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE)
                                .body(new ApiResponse<>(false, "网络连接异常，请稍后重试", data));
        }
        ...
        public ResponseEntity<ApiResponse<Map<String, Object>>> handleCrawlerException(CrawlerException e) {
                logger.error("Crawler error for platform {}: {}", e.getPlatform(), e.getMessage(), e);
                Map<String, Object> data = new HashMap<>();
                data.put("platform", e.getPlatform().name());
                data.put("errorCode", e.getErrorCode());
                return ResponseEntity.badRequest()
                                .body(new ApiResponse<>(false, "爬虫处理异常: " + e.getMessage(), data));
        }
