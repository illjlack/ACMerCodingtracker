...
public class LuoguCrawler {
    ...
    public boolean validateConnection(Map<String, String> cookies) {
        try {
            ExtOjLink link = linkRepo.findById(getOjType()).orElse(null);
            if (link == null || link.getLoginPageLink() == null) {
                logger.warn("洛谷平台链接配置不完整");
                return false;
            }

            // 使用配置的登录页面链接进行验证，尝试访问用户设置页面
            String testUrl = link.getLoginPageLink().replace("/auth/login", "/user/setting");
            ...
            if (statusCode == 200) {
                ...
                try {
                    ...
                    return response != null && !response.contains("登录") && !response.contains("login");
                } catch (Exception e) {
                    logger.debug("洛谷连接内容验证失败: {}", e.getMessage());
                    return false;
                }
            }
        } catch (Exception e) {
            logger.error("验证洛谷连接失败: {}", e.getMessage());
            return false;
        }
    }
